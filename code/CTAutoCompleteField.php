 ...
 */
class CTAutoCompleteField extends TextField {
	 ...
	 * @param string $hiddenTitle [optional] Hidden field name.
	 */
	function __construct($name, $title = null, $value = '', $maxLength = null, $form = null, $sourceClass = null, $sourceField = null, $sourceFilter = null, $hiddenTitle='') {
		// set source
	...
	function getAttributes() {
		return array_merge(
			parent::getAttributes(), array(
				...
				'data-hidden-title' => $this->getHiddenTitle(),
				'autocomplete' => 'off'
			)
		);
	}
    ...
    //echo AUTOCOMPLETEFIELD_DIR.' ';
		Requirements::javascript(AUTOCOMPLETEFIELD_DIR . '/javascript/CTAutoCompleteField.js');
	...
	function Suggest(HTTPRequest $request) {
		...
		foreach ($query as $item) {
			...
			if (!in_array($value, $items)) {
				$items[] = array('id' => $item->ID, 'label'=> $value);
			}
		}
